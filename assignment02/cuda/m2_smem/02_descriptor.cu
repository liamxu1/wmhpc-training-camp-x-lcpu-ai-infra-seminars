...
static uint64_t make_desc(uint32_t saddr, uint32_t lbo, uint32_t sbo,
    ...
    desc |= (static_cast<uint64_t>(saddr) >> 4) << 0;
    desc |= (static_cast<uint64_t>(lbo)   >> 4) << 16;
    desc |= (static_cast<uint64_t>(sbo)   >> 4) << 32;

    // SM100 descriptor version = 1
    desc |= 1ull << 46;

    desc |= (static_cast<uint64_t>(layout) & 0x7ull) << 61;
...
static const uint32_t SCEN[3][3] = {
    {128, 1024, 0},  // K-major, no swizzle
    {0,   1024, 2},  // K-major, 128B swizzle
    {0,   1024, 2},  // MN-major, 128B swizzle
};
