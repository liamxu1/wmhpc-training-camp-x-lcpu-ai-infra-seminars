...
项目 & 你的卡 \\
\midrule
型号 / compute capability & NVIDIA RTX PRO 6000 Blackwell Server Edition / 12.0 \\
SM 数量 & 188\\
warp 大小 & 32\\
...
\begin{enumerate}[label=(\alph*)]
  ...
  错误。因为100TFLOPS是靠大量线程叠加起来的，并不代表单条指令执行的快。
...
因为理论计算出的运算时间考虑的是GPU的吞吐能力，但是实际上串行算法中每一步计算都要等前一步计算结束，有很高的延迟。
...
\midrule
thread & kernel的最小执行单位 & 计算单元上的一个lane & 自己的寄存器 & （自身天然有序） \\
warp & SIMT的基本调度单位，32个thread 同拍执行 & SM中的一个warp & thread寄存器，register file & \_\_shfl\_sync等warp内原语，隐式SIMD同步 \\
block / CTA & 一组协作线程，共享任务分块 & SM内的一组warp & shared memory, L1 Cache & \_\_syncthreads, shared memory读写通信 \\
grid & 整个kernel的全部线程集合 & 多个SM共同执行 & global memory, constant memory等 & kernel级别同步，通过global memory通信 \\
\bottomrule
\end{tabular}
\end{center}
...
SIMD：程序员写向量化的指令。SIMT：程序员写针对单个线程的指令，多线程有独立逻辑，硬件负责打包执行。
错误。依然按warp为单位执行指令，会有warp divergence。
...
\midrule
CPU 单线程 & 4.133 & 0.99 \\
GPU \texttt{<<<1,1>>>} & 166.363 & 39.66 \\
GPU \texttt{<<<1,256>>>} & 2.818 & 0.67 \\
GPU 铺满 grid & 0.021 & 0.01 \\
\bottomrule
\end{tabular}
\end{center}

(a) GPU是为了吞吐设计的，其单指令延迟性能并不好。
(b) 靠的是同时跑海量线程
...
(a) 因为kernel的执行是异步的，kernel返回后GPU还在执行，需要同步后测得的才是真正的完整计算时间。在memcpy到host时隐式同步了。
(b) 我这里从28.8ms变成了33.7ms，系统统一管理产生了额外的页迁移开销？
...
超出了允许的每个block的最大thread数量。kernel launch错误未显式检查，只检查了API返回值。
...
launch规模与数据规模解耦，能处理超大规模数据，但每个线程需要做更多工作，很多SM闲置。
...
block的执行顺序由GPU硬件调度器决定，具体分配到哪个SM、何时开始执行均不可预测，程序正确性不可以依赖block的执行顺序，这一限制正是CUDA scalable programming model的核心：程序员只描述并行任务，硬件自由调度，从而保证程序在不同规模和不同代际的 GPU 上均可正确扩展。
...
\begin{enumerate}[label=(\alph*)]
  ...
  是 3 + 5 * 8 = 43，warpId为1，laneId为11

  \item 这个 block 一共占多少个 warp？

  block一共64个线程，占2个warp。

  \item 若 \texttt{blockDim = (33, 1, 1)}，占几个 warp？这样配置浪费在哪里？

  也是占2个warp，这样第二个warp只有1个线程在工作，造成浪费。
...
预测快两倍，因为按奇偶分的时候发生warp divegence，两个分支分别运行一次。按warp边界对齐分的kernel每个warp都只用走一个分支，运算量大致一半。实际上：warp 内分支 (tid \% 2) : 0.396 ms，按 warp 分支 (tid/32 \% 2):  0.203 ms，比值: 1.95。与预测相似。若计算量一大一小，奇偶分的kernel时间大致为长+短，按warp边界对齐分的kernel运行时间大致按长的来。
...
(a) 因为有的数据还没存入shared memory就被别的线程使用了。 (b) warp内是隐式同步的，但好像不可能落在同一个warp内？或许和底层调度机制相关？
...
理论上应该连续的更好，因为交错的会有warp divergence，同时连续的访存应该也更友好。但是实测似乎差不多，让AI检查也没检查出来什么问题。第三版优化确实性能有所提高。
...
\midrule
register & thread & kernel & 片上 & 编译器 \\
local & thread & kernel & 片外 & 编译器 \\
shared & block & kernel & 片上 & 程序员 \\
global & grid & 应用决定 & 片外 & 程序员 \\
constant & grid & 应用决定 & 片外 & 程序员 \\
L1 / L2 cache & grid & 自动 & 片上 & GPU 硬件 \\
\bottomrule
\end{tabular}
\end{center}
...
global  : PASS  平均 0.0536 ms

constant: PASS  平均 0.0522 ms

这里访存量很小，且global和constant都能轻松命中缓存，优势不明显。constant适用于大量线程反复读取同一只读数据，kernel寄存器压力大，无法缓存参数。
...
naive: PASS  平均 3.5645 ms  (4.71 GB/s)

priv : PASS  平均 0.0159 ms  (1055.20 GB/s)

naive / priv = 224.19x

组内的数据竞争远小于全局的数据竞争，减少了指令串行化，后续全局写需要原子写的次数也大大减少。
...
\midrule
GB/s & 2567.8 & 3141.4 & 2933.5 & 1828.2 & 1848.6 & 988.5 \\
\bottomrule
\end{tabular}
\end{center}

首先stride较小会带来访存合并，从而充分利用带宽，减少内存事务。但是观察到测得的数据并不单调，可能是gpu硬件的自己的缓存起到了作用。
...
occupancy & 100.0\% & 100.0\% & 100.0\% & 83.3\% & 50\% & 16.7\% \\
实测带宽 (GB/s) & 1489.3 & 1491.0 & 1491.5 & 1490.8 & 1461.5 & 748.1 \\
\bottomrule
\end{tabular}
\end{center}
...
(a) 例如18KB的，BLOCK恒定为256，一个SM最大有1536线程，于是这个上限固定为6。考虑shared memory上限，SM上最多100 KB，一个block要用18KB，故最多放5个。occupancy即为 5 * 256 / 1536 约 83.3\%。

(b) 因为这是个强内存受限的kernel，绝大多数时间都在等global访存，常住的warp越多，就有越多的warp可以给GPU调度去隐藏延迟。

(c) 这里是从100\%->83.3\%，几乎没有变化，而后面到50\%开始有约2\%的带宽下降，而到了16\%则下降了近一半。这是因为高occupancy时gpu有足够的warp来隐藏延迟，多一点少一点都差不多，基本都能打满带宽；而低occupancy的时候，pipeline频繁空转，延迟隐藏不住，每少一个可调度的warp都比较致命。
...
cudaEvent计时是kernel耗时，另外两个一个是kernel启动时间，一个是kernel启动+执行（+同步开销）时间。
...
\begin{enumerate}[label=(\alph*)]
  ...
  错误。tile模型是构建在SIMT之上的高层抽象，而非替代关系。
...
\begin{center}\small
\begin{tabular}{@{}l p{7.8em} p{9.2em} p{9.2em} @{}}
\toprule
...
并行单位
  & block 内的 thread
  & \texttt{block}
  & program（逻辑 \texttt{block}） \\
\addlinespace[2pt]
编号
  & \texttt{blockIdx} / \texttt{threadIdx}
  & \texttt{bid = ct.bid(0)}
  & \texttt{pid = tl.program\_id(axis)} \\
\addlinespace[2pt]
数据分工
  & 线程按全局下标划分数据
  & \texttt{block} 通过 tile view 划分数据，每个 \texttt{block} 加载一个 tile
  & program 通过 \texttt{tl.make\_block\_ptr} 划分数据，每个 program 加载一个 tile \\
\addlinespace[2pt]
边界处理
  & \texttt{if} 显式判断
  & tile view + 自动 padding / 裁剪（隐式）
  & mask（\texttt{tl.where}、\texttt{mask=}，隐式） \\
\bottomrule
\end{tabular}
\end{center}
...
每个线程对应哪个/些元素”由编译器决定，这里没有同步，没有计算各种下标，没有寄存器/共享内存手动显式分配，线程的映射与合作方式完全没有体现。
...
集中在计算逻辑部分，因为访存、边界等全都一致，tile视角将计算逻辑与tiling逻辑解耦了。
...
行列号还有，变成了tile的地址；grid尺寸与cuda的类似；边界保护没有了，因为T.copy知道真实shape，会自己推导tile是否越界。
...
规约：两者都是一行命令解决，让编译器来处理如何加和；边界处理：tilelang是编译器隐式处理，但是shared memory多分配的部分还是要自己处理数值的准确性，triton则是要自己来写mask；tile形状则是两者都是用户指定的。
...
生成不匹配的SASS不能运行，生成不匹配的PTX可以正常运行，PTX在程序运行时由CUDA Driver JIT compiler编译成符合的SASS。

\prob[Optional]{8.3}{CONCEPT}
请简单说明 Runtime API 与 Driver API 各自的定位。\texttt{cudaMalloc} 属于哪个？

Runtime API是好用的上层封装，面向应用开发者；Driver API是底层控制接口，面向框架/驱动开发者。cudaMalloc是CUDA Runtime API。
...
% ch=native -DBS=32 -o bin/bonus/matmul_bs32 bonus/matmul.cu &&./bin/bonus/matmul_bs32
% BS=32  平均 0.371 ms  5786.8 GFLOPS
% PASS

% ch=native -DBS=16 -o bin/bonus/matmul_bs16 bonus/matmul.cu &&./bin/bonus/matmul_bs16
% BS=16  平均 0.335 ms  6413.9 GFLOPS
% PASS

% ch=native -DBS=8 -o bin/bonus/matmul_bs8 bonus/matmul.cu &&./bin/bonus/matmul_bs8
% BS=8  平均 0.412 ms  5210.3 GFLOPS
% PASS
...
% BLOCK_M=  32 BLOCK_N=  32 BLOCK_K= 32     0.168 ms   102.0 TFLOPS
% BLOCK_M=  64 BLOCK_N=  64 BLOCK_K= 32     0.114 ms   150.9 TFLOPS
% BLOCK_M= 128 BLOCK_N=  64 BLOCK_K= 32     0.073 ms   234.9 TFLOPS
% BLOCK_M= 128 BLOCK_N= 128 BLOCK_K= 32     0.079 ms   216.8 TFLOPS
% BLOCK_M= 128 BLOCK_N= 128 BLOCK_K= 64     0.102 ms   168.1 TFLOPS
% BLOCK_M=  64 BLOCK_N=  64 BLOCK_K= 64     0.093 ms   184.8 TFLOPS
% torch (cuBLAS)                         0.073 ms   235.3 TFLOPS
...
% 2026-07-31 15:18:25  [TileLang:tilelang.jit.kernel:INFO] (kernel.py:129): TileLang begins to compile kernel `main` with `out_idx=[2]`
% 2026-07-31 15:18:27  [TileLang:tilelang.jit.kernel:INFO] (kernel.py:137): TileLang completes to compile kernel `main`
% block_M=  64 block_N=  64 block_K= 32 threads=128 stages=1     0.229 ms    75.1 TFLOPS
% 2026-07-31 15:18:27  [TileLang:tilelang.jit.kernel:INFO] (kernel.py:129): TileLang begins to compile kernel `main` with `out_idx=[2]`
% 2026-07-31 15:18:28  [TileLang:tilelang.jit.kernel:INFO] (kernel.py:137): TileLang completes to compile kernel `main`
% block_M=  64 block_N=  64 block_K= 32 threads=128 stages=3     0.128 ms   134.5 TFLOPS
% 2026-07-31 15:18:29  [TileLang:tilelang.jit.kernel:INFO] (kernel.py:129): TileLang begins to compile kernel `main` with `out_idx=[2]`
% 2026-07-31 15:18:30  [TileLang:tilelang.jit.kernel:INFO] (kernel.py:137): TileLang completes to compile kernel `main`
% block_M= 128 block_N= 128 block_K= 32 threads=128 stages=3     0.096 ms   178.2 TFLOPS
% 2026-07-31 15:18:30  [TileLang:tilelang.jit.kernel:INFO] (kernel.py:129): TileLang begins to compile kernel `main` with `out_idx=[2]`
% 2026-07-31 15:18:31  [TileLang:tilelang.jit.kernel:INFO] (kernel.py:137): TileLang completes to compile kernel `main`
% block_M= 128 block_N= 128 block_K= 64 threads=256 stages=3     0.084 ms   205.6 TFLOPS
% 2026-07-31 15:18:32  [TileLang:tilelang.jit.kernel:INFO] (kernel.py:129): TileLang begins to compile kernel `main` with `out_idx=[2]`

\end{enumerate}

\begin{table}[htbp]
\centering
\caption{不同 MatMul 实现的性能对比}
\label{tab:matmul-comparison}
\begin{tabular}{@{}llcc@{}}
\toprule
\textbf{实现方式} & \textbf{最佳配置} & \textbf{峰值吞吐} & \textbf{相对 cuBLAS} \\
\midrule
Naive CUDA   & \texttt{BS=16}                                & 6.4 TFLOPS  & 2.7\%  \\
Triton       & \texttt{BM=128, BN=64, BK=32}                & 234.9 TFLOPS & 99.8\% \\
TileLang     & \texttt{BM=128, BN=128, BK=64, threads=256, stages=3} & 205.6 TFLOPS & 87.4\% \\
cuBLAS (torch) & --                                            & 235.3 TFLOPS & 100\% \\
\bottomrule
\end{tabular}
\end{table}
...
虽然 TileLang 提供了比 Triton 更细粒度的硬件控制能力，但在本实验中并未转化为性能优势。主要原因包括：Triton 拥有经过大量硬件验证的自动调度策略，能够在寄存器压力、shared memory 布局、warp 利用率和 pipeline 深度之间自动取得较优平衡；而 TileLang 将更多调度决策暴露给用户，若缺乏精细的 cost model 和自动搜索机制，容易陷入次优配置空间（如过大的 block、过高的线程数或不匹配的 tile shape）。因此，在同等人工调参条件下，Triton 的表现反而更稳定、更接近 cuBLAS。
